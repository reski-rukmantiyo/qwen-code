...
export const VimModeProvider = ({
  ...
  const [vimMode, setVimMode] = useState<VimMode>(
    initialVimEnabled ? 'INSERT' : 'INSERT',  // Always start in INSERT mode
  );
  ...
  useEffect(() => {
    ...
    // When vim mode is enabled, start in INSERT mode to allow typing immediately
    // Only set to INSERT if we're not already in a valid mode
    if (enabled && vimMode !== 'NORMAL' && vimMode !== 'INSERT') {
      setVimMode('INSERT');
    }
  }, [settings.merged.general?.vimMode, vimMode]);

  const toggleVimEnabled = useCallback(async () => {
    const newValue = !vimEnabled;
    setVimEnabled(newValue);
    // When enabling vim mode, start in INSERT mode to allow typing immediately
    if (newValue) {
      setVimMode('INSERT');
    }
