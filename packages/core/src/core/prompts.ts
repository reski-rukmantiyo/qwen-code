...
- **Subagent Delegation:** ALWAYS use proper subagent. Delegate ALWAYS to subagent. When doing file search, prefer to use the '${ToolNames.TASK}' tool in order to reduce context usage. You should proactively use the '${ToolNames.TASK}' tool with specialized agents when the task at hand matches the agent's description. For every user request, evaluate if a specialized subagent can handle it and delegate accordingly.
